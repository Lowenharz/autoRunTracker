...
namespace illumina { namespace interop { namespace model { namespace summary
{
    ...
    public:
        ...
        stat_summary(const size_t channel_count) :
                ...
                // Compass
                m_base_percent(constants::NUM_OF_BASES, std::numeric_limits<float>::quiet_NaN()),
    ...
    public://IUO
        /** @defgroup stat_summary_iuo Stat summary (IUO)
         *
         * Summary of key metrics for Illumina Use Only
         *
         * @ingroup read_summary
         * @ref illumina::interop::model::summary::lane_summary "See full class description"
         * @ref illumina::interop::model::summary::surface_summary "See full class description"
        ...
        /** Get the percent of bases greater than or equal to Q30 in the last 10 usable cycles of a read
         *
         * @note IUO
         * @return percent of bases greater than or equal to Q30 in the last 10 usable cycles of a read
         */
         ...
         *
         * @note IUO
         * @return mean summarizing the percent occupied
         */
        float percent_occupied() const
        {
            return m_percent_occupied;
        }
        /** Get statistics summarizing the rotation about the z-axis (first channel only)
         *
         * @note IUO
         * @return statistics summarizing the rotation about the z-axis (first channel only)
         */
        const metric_stat_t &theta_z() const
        {
            return m_theta_z;
        }
        /** Get statistics summarizing the rotation of the affine transform (first channel only)
         *
         * @note IUO
         * @return statistics summarizing the rotation of the affine transform (first channel only)
         */
        const metric_stat_t &theta() const
        {
            return m_theta;
        }
        ...
        // Compass
        //--------------------------------------------------------------------------------------------------------------
        /** Get the FWHM value for the lane
         *
         * @note IUO Compass
         * @return FWHM value for lane
         */
        const std::vector<float>& fwhm() const
        {
            return m_fwhm;
        }

        /** Get the percent base values by base by lane
         *
         * @note IUO Compass
         * @return percent base values
         */
        const std::vector<float>& percent_base() const
        {
            return m_base_percent;
        }
    public:
        /** Set the percent of bases greater than or equal to Q30 in the last 10 usable cycles of a read
         *
         * @param val percent of bases greater than or equal to Q30 in the last 10 usable cycles of a read
         */
        ...
        /** Set the FWHM value by lane
         *
         * @note IUO Compass
         * @param channel channel index
         * @param val FWHM value averaged by lane
         */
        void fwhm(const size_t channel, const float val)
        {
            INTEROP_ASSERT(channel < m_fwhm.size());
            m_fwhm[channel] = val;
        }

        /** Set the base percent values, by base, by lane
         *
         * @note IUO Compass
         * @param index base index
         * @param val base percent value
         */
        void percent_base(const size_t index, const float val)
        {
            INTEROP_ASSERT(index < m_base_percent.size());
            m_base_percent[index] = val;
        }
        /** Set mean summarizing the percent occupied
         *
         * @note IUO
         * @param val mean summarizing the percent occupied
         */
         ...
         *
         * @note IUO
         * @param stat statistics summarizing the rotation about the z-axis (first channel only)
         */
        void theta_z(const metric_stat_t& stat)
        {
            m_theta_z = stat;
        }
        /** Get statistics summarizing the rotation of the affine transform (first channel only)
         *
         * @note IUO
         * @param stat statistics summarizing the rotation of the affine transform (first channel only)
         */
        void theta(const metric_stat_t& stat)
        {
            m_theta = stat;
        }
    ...
    public:
         ...
         */
        void resize_stat(const size_t channel_count)
        {
            m_fwhm.assign(channel_count, std::numeric_limits<float>::quiet_NaN());
            m_base_percent.assign(constants::NUM_OF_BASES, std::numeric_limits<float>::quiet_NaN());
        }
    ...
    private:
        // IUO
        metric_stat_t m_theta_z;
        metric_stat_t m_theta;
        float m_percent_gt_q30_last_10_cycles;
        float m_percent_occupied;

    private:
        // Compass
        std::vector<float> m_base_percent;
