...
namespace illumina { namespace interop { namespace io { namespace plot
{
    ...
    public:
        ...
        /** Write the subtile heat map of a specific metric to the output stream using the GNUPlot format
         *
         * @note IUO
         * @param out output stream
         * @param data subtile heatmap data
         * @param output_image_path optional output image path for the script
         */
        void write_subtile(std::ostream& out,
                           const model::plot::subtile_data& data,
                           const std::string& output_image_path="")
        {
            write_output_format(out, output_image_path);
            write_title(out, data);
            ...
            out << "unset tics\n";
            out << "unset border\n";
            out << "set cbrange [" << table::handle_nan(data.saxis().min()) << ":" << table::handle_nan(data.saxis().max()) << "]\n";
            out << "set palette defined (0 \"blue\", 0.286 \"aquamarine\", 0.429 \"light-green\", 0.572 \"green\", 0.714 \"greenyellow\", 0.858 \"yellow\", 1 \"orange\")\n";
            out << "set autoscale fix\n";
            out << "set size ratio 2\n";
            out << "set yrange[:] reverse\n";
            out << "set datafile separator \",\"\n";
            out << "plot \"-\" matrix with image" << "\n";
            const size_t num_x_bins = data.subtile_count_x();
            const size_t num_y_bins = data.subtile_count_y();
            const size_t swath_count = data.column_count()/data.tile_count()/num_x_bins/num_y_bins;
            for(size_t y=0;y<data.tile_count();++y)
            {
                for(size_t bin_y=0; bin_y < num_y_bins; ++bin_y)
                {
                    for (size_t x = 0; x < data.row_count(); ++x)
                    {
                        for (size_t s = 0; s < swath_count; ++s)
                        {
                            for (size_t bin_x = 0; bin_x < num_x_bins; ++bin_x)
                            {
                                if(x > 0 || s > 0 || bin_x > 0) out << ",";
                                out << table::handle_nan(data(x, (y + s * data.tile_count()), bin_x, bin_y));
                            }
                        }
                        out << ",nan";
                    }
