...
#       undef INTEROP_TUPLE1
     ...
     * @return metric feature type
     */
    inline constants::metric_feature_type to_feature(const constants::event_type type)
    {
        using namespace constants;
        // TODO: This can be reduced to a single macro define
        typedef std::pair<event_type, metric_feature_type > mapped_t;
#       define INTEROP_TUPLE_ASSIGN_AND_FEATURE(Metric, Ignore1, Feature) mapped_t(Metric,static_cast<metric_feature_type>(Feature))
#       define INTEROP_TUPLE_FEATURE(Metric, Feature) mapped_t(Metric,static_cast<metric_feature_type>(Feature))
        static const mapped_t name_types[] = {INTEROP_ENUM_EVENT_TYPE};
#       undef INTEROP_TUPLE_FEATURE
#       undef INTEROP_TUPLE_ASSIGN_AND_FEATURE
        return util::constant_mapping_get(name_types, type, UnknownMetricFeature);
    }
    /** Test if metric type is produced for a region within a tile
     *
     * @param type metric type
     * @return true if metric is produced every a region within a tile
     */
    inline bool is_subtile_metric(const constants::metric_type type)
    {
        return (to_feature(type) & constants::SubtileFeature) == constants::SubtileFeature;
    }


    /** Test if the event type is consistent with storing a time interval
     *
     * @param type event type
     * @return true if the event type is consistent with storing a time interval
     */
    inline bool is_interval_event(const constants::event_type type)
    {
        return (to_feature(type) & constants::IntervalFeature) == constants::IntervalFeature;
    }

    /** Test if metric type is Illumina Use Only
     *
     * @param type metric type
     * @return true if metric is Illumina Use Only
     */
    inline bool is_iuo_metric(const constants::metric_type type)
    {
        return (to_feature(type) & constants::IUOFeature) == constants::IUOFeature;
    }
    /** Test if group type is Illumina Use Only
     *
     * @param type group type
     * @return true if group is Illumina Use Only
     */
    inline bool is_iuo(const constants::metric_group type)
    {
        return (to_feature(type) & constants::IUOFeature) == constants::IUOFeature;
    }
