...
namespace illumina { namespace interop { namespace logic { namespace summary
{
    ...
    public:
        ...
        qval_cache(const model::summary::run_summary &run, const size_t surface_count=1) :
                ...
                /*IUO*/
                m_read_lane_cache_last_10_cycle(run.size(), qval_total_vector_t(run.lane_count()*surface_count))
        {}
        /** Accumulate a metric into the cache
         *
         * @param metric collapsed q-metric
         * @param cycle_to_read mapping between cycle and read information (IUO)
                 ...
                 const size_t surface=0)
        {
            ...
            /** IUO */
            const size_t cycle = static_cast<size_t>(metric.cycle());
            const size_t cycle_plus_10 = cycle + 10;
            if(cycle_plus_10 >= cycle_to_read.size() ||
                    ((cycle_plus_10 < cycle_to_read.size()) &&
                            (cycle_to_read[cycle-1].number != cycle_to_read[cycle_plus_10].number)))
            {
                m_read_lane_cache_last_10_cycle[read_number][lane_surface_index].above_qval += metric.q30();
                m_read_lane_cache_last_10_cycle[read_number][lane_surface_index].total += metric.total();
            }
        }
        ...
        /** Get qval_total for last 10 cycles struct for given read/lane/surface
         *
         * @note IUO
         * @param read_number read index
         * @param lane lane index
         * @param surface surface index
         * @return qval_total struct for last 10 cycles
         */
        const qval_total& total_for_last_10(const size_t read_number, const size_t lane, const size_t surface=0)const
        {
            INTEROP_ASSERT(read_number < m_read_lane_cache_last_10_cycle.size());
            const size_t index = index_of(lane, surface);
            INTEROP_ASSERT(index < m_read_lane_cache_last_10_cycle[read_number].size());
            return m_read_lane_cache_last_10_cycle[read_number][index];
        }
    ...
    private:
        ...
        /** IUO */
        qval_total_vector2d_t m_read_lane_cache_last_10_cycle;
    };
    ...
    */
                                             ...
                                             throw( model::index_out_of_bounds_exception )
    {
        ...
        for(;beg != end;++beg)
        {
            ...
            read_lane_cache.add(*beg, cycle_to_read /*iuo*/, read_number, lane);

            if(surface_count < 2) continue;
            const size_t surface = beg->surface(naming_method);
            INTEROP_ASSERT(surface > 0);
            read_lane_surface_cache.add(*beg, cycle_to_read /*iuo*/, read_number, lane, surface-1);
        }
        ...
        /** IUO */
        ::uint64_t total_useable_calls_last_10_cycles = 0;
        ::uint64_t useable_calls_gt_q30_last_10_cycles = 0;
        ::uint64_t total_useable_calls_nonindex_last_10_cycles = 0;
        ::uint64_t useable_calls_gt_q30_nonindex_last_10_cycles = 0;
        for(size_t read=0;read<run.size();++read)
        {
            ...
            /** IUO */
            ::uint64_t total_useable_calls_by_read_last_10_cycles=0;
            ::uint64_t useable_calls_gt_q30_by_read_last_10_cycles = 0;
            for (size_t lane = 0; lane < run[read].size(); ++lane)
            {
                ...
                if( total_metrics_in_lane_read > 0)
                {
                    ...
                    //IUO
                    const qval_total& curr_tot_for_last_10 = read_lane_cache.total_for_last_10(read, lane);
                    total_useable_calls_by_read_last_10_cycles += curr_tot_for_last_10.total;
                    useable_calls_gt_q30_by_read_last_10_cycles += curr_tot_for_last_10.above_qval;
                    stat_summary.percent_gt_q30_last_10_cycles(curr_tot_for_last_10.percent_above_qval());
                    //IUO
                }
                else
                {
                ...
                for(size_t surface=0;surface<surface_count;++surface)
                {
                    ...
                    if( total_in_surface_lane_read > 0)
                    {
                        ...
                        //IUO
                        const qval_total& curr_tot_for_last_10 = read_lane_surface_cache.total_for_last_10(read, lane, surface);
                        stat_summary_surface.percent_gt_q30_last_10_cycles(curr_tot_for_last_10.percent_above_qval());
                        //IUO
                    }
                    else
                    {
            ...
            /** IUO */
            run[read].summary().percent_gt_q30_last_10_cycles(100 * divide(float(useable_calls_gt_q30_by_read_last_10_cycles), float(total_useable_calls_by_read_last_10_cycles)));
            total_useable_calls_last_10_cycles += total_useable_calls_by_read_last_10_cycles;
            useable_calls_gt_q30_last_10_cycles += useable_calls_gt_q30_by_read_last_10_cycles;
            ...
            if(!run[read].read().is_index())
            {
                ...
                /** IUO */
                total_useable_calls_nonindex_last_10_cycles += total_useable_calls_by_read_last_10_cycles;
                useable_calls_gt_q30_nonindex_last_10_cycles += useable_calls_gt_q30_by_read_last_10_cycles;
            }
        }
        ...
        /** IUO */
        run.nonindex_summary().percent_gt_q30_last_10_cycles(100 * divide(float(useable_calls_gt_q30_nonindex_last_10_cycles), float(total_useable_calls_nonindex_last_10_cycles)));
        run.total_summary().percent_gt_q30_last_10_cycles(100 * divide(float(useable_calls_gt_q30_last_10_cycles), float(total_useable_calls_last_10_cycles)));
    }
}}}}
