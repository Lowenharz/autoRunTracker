...
#       undef INTEROP_TUPLE7 // Reuse this for another conversion
    ...
    private:
        ...
        template<typename OutputIterator>
        static void
        copy_to(OutputIterator destination, OutputIterator end, const model::metrics::point2d& source, const size_t num_digits)
        {
            (void) end;
            assign(*destination, source.x(), num_digits);
            ++destination;
            assign(*destination, source.y(), num_digits);
        }
