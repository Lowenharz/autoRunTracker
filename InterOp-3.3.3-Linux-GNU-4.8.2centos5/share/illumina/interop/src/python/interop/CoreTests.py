...
class CoreTests(unittest.TestCase):
    ...
    def test_invalid_metric_type(self):
        ...
        try:
            py_interop_run_metrics.list_metrics_to_load("Unknown", valid_to_load, py_interop_run.NovaSeq, True)
    ...
    def test_list_distortion_types(self):
        """
        Test if the distortion types enum matches the expected types
        """
        ...
        py_interop_run.list_distortion_type(distortionTypes)
        expected=["XFieldOfViewYDistortion", "XFieldOfViewXDistortion", "Radial"]
        self.assertEqual(len(expected)+1, len(distortionTypes))
        for i in xrange(len(expected)):
            self.assertEqual(distortionTypes[i], expected[i])

    def test_to_event_feature(self):
        """
        Test if event logic is properly wrapped
        """

        event_type = py_interop_run.RtaStarted
        self.assertEqual(py_interop_run_metrics.to_event_feature(event_type), 128)

    def test_count_event_table_columns(self):
        """
        Test if event logic is properly wrapped
        """

        columns = py_interop_table.event_column_vector()
        self.assertEqual(py_interop_table.count_event_table_columns(columns), 0)
