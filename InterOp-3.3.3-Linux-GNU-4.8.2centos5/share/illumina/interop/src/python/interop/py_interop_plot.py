...
class filter_options(_object):
    ...
    def supports_section(self, plot_type, info):
        return _py_interop_plot.filter_options_supports_section(self, plot_type, info)

    def supports_swath(self, plot_type):
        return _py_interop_plot.filter_options_supports_swath(self, plot_type)

    def supports_tile(self, plot_type):
        return _py_interop_plot.filter_options_supports_tile(self, plot_type)
...
class subtile_data(flowcell_data):
    __swig_setmethods__ = {}
    for _s in [flowcell_data]:
        __swig_setmethods__.update(getattr(_s, '__swig_setmethods__', {}))
    __setattr__ = lambda self, name, value: _swig_setattr(self, subtile_data, name, value)
    __swig_getmethods__ = {}
    for _s in [flowcell_data]:
        __swig_getmethods__.update(getattr(_s, '__swig_getmethods__', {}))
    __getattr__ = lambda self, name: _swig_getattr(self, subtile_data, name)
    __repr__ = _swig_repr

    def __init__(self):
        this = _py_interop_plot.new_subtile_data()
        try:
            self.this.append(this)
        except __builtin__.Exception:
            self.this = this
    __swig_destroy__ = _py_interop_plot.delete_subtile_data
    __del__ = lambda self: None

    def set_buffer(self, *args):
        return _py_interop_plot.subtile_data_set_buffer(self, *args)

    def resize(self, lanes, swaths, tiles, subtile_count_x, subtile_count_y):
        return _py_interop_plot.subtile_data_resize(self, lanes, swaths, tiles, subtile_count_x, subtile_count_y)

    def set_data(self, lane_idx, tile_idx, bin_x, bin_y, tile_id, value):
        return _py_interop_plot.subtile_data_set_data(self, lane_idx, tile_idx, bin_x, bin_y, tile_id, value)

    def populate_tile_data_at(self, buffer, lane_idx, tile_idx):
        return _py_interop_plot.subtile_data_populate_tile_data_at(self, buffer, lane_idx, tile_idx)

    def at(self, row, col, bin_x, bin_y):
        return _py_interop_plot.subtile_data_at(self, row, col, bin_x, bin_y)

    def tile_id(self, row, col, bin_x, bin_y):
        return _py_interop_plot.subtile_data_tile_id(self, row, col, bin_x, bin_y)

    def subtile_count_x(self):
        return _py_interop_plot.subtile_data_subtile_count_x(self)

    def subtile_count_y(self):
        return _py_interop_plot.subtile_data_subtile_count_y(self)

    def clear(self):
        return _py_interop_plot.subtile_data_clear(self)

    def index_of(self, row, col, bin_x, bin_y):
        return _py_interop_plot.subtile_data_index_of(self, row, col, bin_x, bin_y)
subtile_data_swigregister = _py_interop_plot.subtile_data_swigregister
subtile_data_swigregister(subtile_data)
...
def list_by_cycle_metrics(types, show_iuo, ignore_accumulated=False):
    return _py_interop_plot.list_by_cycle_metrics(types, show_iuo, ignore_accumulated)
list_by_cycle_metrics = _py_interop_plot.list_by_cycle_metrics

def filter_by_cycle_metrics(types, show_iuo, ignore_accumulated=False):
    return _py_interop_plot.filter_by_cycle_metrics(types, show_iuo, ignore_accumulated)
...
def list_by_lane_metrics(types, show_iuo, ignore_pf=False):
    return _py_interop_plot.list_by_lane_metrics(types, show_iuo, ignore_pf)
list_by_lane_metrics = _py_interop_plot.list_by_lane_metrics

def filter_by_lane_metrics(types, show_iuo, ignore_pf=False):
    return _py_interop_plot.filter_by_lane_metrics(types, show_iuo, ignore_pf)
...
def list_flowcell_metrics(types, show_iuo, ignore_accumulated=False):
    return _py_interop_plot.list_flowcell_metrics(types, show_iuo, ignore_accumulated)
list_flowcell_metrics = _py_interop_plot.list_flowcell_metrics

def filter_flowcell_metrics(types, show_iuo, ignore_accumulated=False):
    return _py_interop_plot.filter_flowcell_metrics(types, show_iuo, ignore_accumulated)
...
def find_first_valid(*args):
    return _py_interop_plot.find_first_valid(*args)
find_first_valid = _py_interop_plot.find_first_valid

def select_tile_values(*args):
    return _py_interop_plot.select_tile_values(*args)
select_tile_values = _py_interop_plot.select_tile_values

def plot_subtile_map(*args):
    return _py_interop_plot.plot_subtile_map(*args)
plot_subtile_map = _py_interop_plot.plot_subtile_map

def list_subtile_metrics(*args):
    return _py_interop_plot.list_subtile_metrics(*args)
list_subtile_metrics = _py_interop_plot.list_subtile_metrics

def calculate_subtile_buffer_size(metrics, type, options):
    return _py_interop_plot.calculate_subtile_buffer_size(metrics, type, options)
calculate_subtile_buffer_size = _py_interop_plot.calculate_subtile_buffer_size

def plot_subtile_aggregate(*args):
    return _py_interop_plot.plot_subtile_aggregate(*args)
plot_subtile_aggregate = _py_interop_plot.plot_subtile_aggregate

def plot_distortion(metrics, options, distortion_curve_index, data):
    return _py_interop_plot.plot_distortion(metrics, options, distortion_curve_index, data)
plot_distortion = _py_interop_plot.plot_distortion

def plot_event_by_cycle(metrics, options, data):
    return _py_interop_plot.plot_event_by_cycle(metrics, options, data)
plot_event_by_cycle = _py_interop_plot.plot_event_by_cycle

def list_interval_events(types):
    return _py_interop_plot.list_interval_events(types)
list_interval_events = _py_interop_plot.list_interval_events

def plot_event_by_event_type(metrics, options, data, save_outliers=True):
    return _py_interop_plot.plot_event_by_event_type(metrics, options, data, save_outliers)
plot_event_by_event_type = _py_interop_plot.plot_event_by_event_type

def plot_run_diagnostics_by_cycle(metrics, options, data):
    return _py_interop_plot.plot_run_diagnostics_by_cycle(metrics, options, data)
plot_run_diagnostics_by_cycle = _py_interop_plot.plot_run_diagnostics_by_cycle

def plot_run_diagnostics_flowcell_map(*args):
    return _py_interop_plot.plot_run_diagnostics_flowcell_map(*args)
plot_run_diagnostics_flowcell_map = _py_interop_plot.plot_run_diagnostics_flowcell_map

def plot_run_diagnostics_flowcell_map2(*args):
    return _py_interop_plot.plot_run_diagnostics_flowcell_map2(*args)
plot_run_diagnostics_flowcell_map2 = _py_interop_plot.plot_run_diagnostics_flowcell_map2

def calculate_run_diagnostics_flowcell_buffer_size(metrics, options):
    return _py_interop_plot.calculate_run_diagnostics_flowcell_buffer_size(metrics, options)
calculate_run_diagnostics_flowcell_buffer_size = _py_interop_plot.calculate_run_diagnostics_flowcell_buffer_size
